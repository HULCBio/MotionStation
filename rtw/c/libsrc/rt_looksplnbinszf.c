/* 
 * File: rt_looksplnbinszf.c generated from file
...
real32_T rt_LookSplNBinSZf(int_T                numDims,
			   const real32_T * u,
			   rt_LUTSplineWork  * SWork)
{
  ...
  for(k=0; k < numDims; k++) {
    ...
    bpIndex[k] = rt_PLookBinXf(u[k], &bpLambda[k], bpData, maxIndex[k], maxIndex[k] >> 1);
  }
                     
  return(rt_IntrpNSplf(numDims, SWork, 3));
}
/* [EOF] rt_looksplnbinszf.c */
