 ...
 *
 * File: rt_i32zcfcn.c     $Revision: 1.3 $
 *
 * Abstract:
 ...
 */
ZCEventType rt_I32ZCFcn(ZCDirection direction,
                       ...
                       int32_T      zcSig)
{
    ...
    if (prevSigState == UNINITIALIZED_ZCSIG) {
         ...
         * previous zc signal.
         */
        *prevSigStatePtr = ((zcSig < 0)? NEG_ZCSIG:
                            (zcSig > 0? POS_ZCSIG: ZERO_ZCSIG));
    } else {
        /*
         * Update sampled zero crossing events
         */
        ZCSigState sigState = ((zcSig < 0)? NEG_ZCSIG:
                               (zcSig > 0? POS_ZCSIG: ZERO_ZCSIG));
