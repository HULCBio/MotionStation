/*
 * File : rt_intrpnflatcd.c generated from file
 ...
 *
 */
real_T rt_IntrpNFlatCd(int_T                     numDims,
		       const rt_LUTnWork * const TWork)
{
  const real_T   * const tableData = TWork->tableData;
  ...
  for (k=0; k < numDims; k++) {
    ...
    if (addOff < 0) {
      ...
      if (bpLambda[k] >= 1.0) addOff++;
    }
    addOff *= dimSizes[k];
    offset += addOff;
  }
  return ( tableData[offset] );
} /* [EOF] rt_intrpnflatcd.c */
