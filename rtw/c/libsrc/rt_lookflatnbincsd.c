/* 
 * File: rt_lookflatnbincsd.c generated from file
...
real_T rt_LookFlatNBinCSd(int_T                numDims,
			  const real_T * u,
			  rt_LUTnWork  * TWork)
{
  ...
  for(k=0; k < numDims; k++) {
    ...
    bpIndex[k] = rt_PLookBinCd(u[k], &bpLambda[k], bpData, maxIndex[k], bpIndex[k]);
  }
                     
  return(rt_IntrpNFlatd(numDims-1, TWork));
}
/* [EOF] rt_lookflatnbincsd.c */
