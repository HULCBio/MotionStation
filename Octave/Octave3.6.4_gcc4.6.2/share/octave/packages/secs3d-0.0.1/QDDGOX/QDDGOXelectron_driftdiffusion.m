...
%   n=QDDGOXelectron_driftdiffusion(mesh,Dsides,Intsides,nin,pin,V,un,tn,tp,n0,p0)
%   IN:
...
function n=QDDGOXelectron_driftdiffusion(mesh,Dsides,Intsides,nin,pin,V,un,tn,tp,n0,p0)
...
Intnodes=Ugetnodesonface(mesh,Intsides)';

denom = (tp*(nin+sqrt(n0.*p0))+tn*(pin+sqrt(n0.*p0)));
u     = un;
M     = pin./denom;
M(Intnodes)=M(Intnodes)+1;
U     = p0.*n0./denom;
U(Intnodes)=U(Intnodes)+nin(Intnodes);
guess = nin;
