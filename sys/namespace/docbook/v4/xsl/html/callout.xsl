...
<xsl:template match="areaspec">
  <xsl:element name="map">
    <!-- why can't I use template object.id here?-->
...
<xsl:template match="area">
  ...
  <xsl:if test="count($targets) > 0">
     ...
     <area coords="{@coords}" shape="{@otherunits}">
        <xsl:attribute name="href">
          <xsl:call-template name="href.target">
           <xsl:with-param name="object" select="$targets[1]"/>
          </xsl:call-template>
        </xsl:attribute>
     </area>
...
<xsl:template match="graphicco">
   <xsl:choose>
    <xsl:when test="child::areaspec">
    <xsl:apply-templates select="areaspec"/>
		<xsl:element name="img">
			<xsl:attribute name="src"><xsl:value-of select="child::graphic//@fileref"/></xsl:attribute>
			<xsl:attribute name="usemap">#<xsl:value-of select="generate-id(child::areaspec)"/></xsl:attribute>
