...
<xsl:template match="tgroup" name="tgroup">
  ...
  <table>
    ...
    <!-- @CHANGE: there was a bug in the stylesheet which placed the footer before the body -->
...
<xsl:template match="row">
  ...
  <tr>
    ...
    <!-- CHANGE valign with top by default.
    Note that I would like to put this at the TABLE level, but
    IE doesn't honor table valigns.  was:
    <xsl:if test="@valign">
      <xsl:attribute name="valign">
        <xsl:value-of select="@valign"/>
      </xsl:attribute>
      ...
      <xsl:element name="{$cellgi}">
        ...
        <xsl:if test="$table.borders.with.css != 0">
          <xsl:attribute name="style">
            ...
            <!-- @CHANGE have bottom borders if the last row of a theader
            <xsl:if test="$rowsep &gt; 0">
                 @NOTE: should also add check for (ancestor::tbody and count(../following-sibling::row)=0)
             -->
            <xsl:if test="($rowsep &gt; 0) or (ancestor::thead and count(../following-sibling::row)=0)">
            <!-- @END CHANGE -->
              <xsl:call-template name="border">
                <xsl:with-param name="side" select="'bottom'"/>
              </xsl:call-template>
    ...
    <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="$colspec.colnum=$countcol">
          <col>
          <!-- @CHANGE - get colwidth without extensions
