%function [mubnd,var,dc,dr,gcr,grc] = gebubd(m,v,blk,diagnose,clevel,Dinitbnd,Ginitbnd,pmu)
...
function [mubnd,var,dc,dr,gcr,grc] = gebubd(m,v,blk,diagnose,clevel,Dinitbnd,Ginitbnd,pmu)
    ...
    [la,ra,pa,qa,wa,sa,ha,ja,ca,da,ta,lb,rb,pb,qb,wb,sb,hb,jb,cb,db,tb,...
        tx,ty,tz,tv,dgpoint] = gcnvert(m,v,blk);
    ...
    lastiters = inf;
while (maingo == 1 | cnt<1) & resetbad<1 & abstop==0
    cnt = cnt+1;
    ...
    while gogo==1 & iter<(2*lastiters+5)
        ...
        x2=clock;
        btime = btime + etime(x2,x1);
        x1=clock;
        step = Hall\Gall;
        resid = Hall*step-Gall;
        if norm(resid)/norm(Gall)>1e-3
            disp('Bad resid')
            gogo = 0;
        end

        x2=clock;
        invhtime = invhtime + etime(x2,x1);
    ...
    if iter<2*lastiters+5
        ...
        if diagnose>=1
            disp([cnt iter lastiters lam lowerbnd sqrt(lam)])
        end
        theta = 0.01;
        ...
        % convergence problems, bail out using previous values
        abstop = 1;
    end
end
...
mubnd = sqrt(lam);
if diagnose==1
    disp([btime ghtime invhtime gbtime dbtime]);
end
%
%