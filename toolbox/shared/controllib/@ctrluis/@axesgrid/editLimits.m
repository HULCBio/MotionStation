...
%   $Revision: 1.1.6.2 $  $Date: 2004/04/11 00:16:43 $
...
Lim1Str = get(s.LimRows(RCIndex).Lim1,'Text');  % current string
Lim2Str = get(s.LimRows(RCIndex).Lim2,'Text');
...
if ~isempty(Lim1) & ~isempty(Lim2) & Lim1<Lim2 & ~isequal(CurrentLims,[Lim1 Lim2])
   ...
   try
      if ~isempty(s.RCSelect.getParent) & s.RCSelect.getSelectedIndex>0
         ...
         feval(limfcn,AxGrid,[Lim1 Lim2],[s.RCSelect.getSelectedIndex RCIndex])
      elseif RCSize(2)==1
         % Global setting
         ...
         for ct=1:RCSize(1)
            feval(limfcn,AxGrid,[Lim1 Lim2],[ct RCIndex])  % all subgrid peers
         end
      end
