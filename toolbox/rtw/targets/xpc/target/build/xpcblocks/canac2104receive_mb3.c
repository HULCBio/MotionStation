/* $Revision: 1.3 $ $Date: 2002/03/25 04:00:34 $ */
/* canac2104receive_mb3.c - xPC Target, non-inlined S-function driver for CAN-AC2-104 from Softing (multi board)  */
...
#define 	S_FUNCTION_NAME 	canac2104receive_mb3
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	if (((int_T)mxGetPr(CAN_PORT_ARG)[0])==1) {		// CAN 1
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
			...
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data_mb3 CAN1: %d\n",frc);
				return;
			} 
    	...
    	for (i=0;i<mxGetN(CAN_INDEX_ARG);i++) {
			...
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data_mb3 CAN2: %d\n",frc);
				return;
			} 
            value=(double *) &p_data[0];
			y[0]=*value;
