/* $Revision: 1.6 $ $Date: 2002/03/25 04:03:36 $ */
/* canac2pcisetup_mb3.c - xPC Target, non-inlined S-function driver for CAN-AC2-PCI from Softing (multi board)  */
...
#define S_FUNCTION_NAME canac2pcisetup_mb3
...
extern int CANAC2_FIRMWARE_LOADED_mb3;

extern int CANAC2_1_send_mb3[MAX_OBJECTS];
extern int CANAC2_1_receive_mb3[MAX_OBJECTS];
extern int CANAC2_2_send_mb3[MAX_OBJECTS];
extern int CANAC2_2_receive_mb3[MAX_OBJECTS];

extern int CANAC2_1_sendmaxindex_mb3;
extern int CANAC2_1_sendindex_mb3[MAX_OBJECTS];
extern int CANAC2_1_receivemaxindex_mb3;
extern int CANAC2_1_receiveindex_mb3[MAX_OBJECTS];
extern int CANAC2_2_sendmaxindex_mb3;
extern int CANAC2_2_sendindex_mb3[MAX_OBJECTS];
extern int CANAC2_2_receivemaxindex_mb3;
extern int CANAC2_2_receiveindex_mb3[MAX_OBJECTS];
...
static int Initialize_can_parameter(SimStruct *S)
{
   ...
   //printf("CANPC_reset_chip_mb3 OK\n\n");
   ...
   // Set Bit timing for CAN 1 and 2
   	...
   	//printf("CANPC_initialize_chip_mb3 OK\n");
	...
	baud=(int)mxGetPr(CAN2_BAUDRATE_ARG)[0];
   	...
   	//printf("CANPC_initialize_chip2_mb3 OK\n");
	...
	// Set acceptance filter for CAN 1 and 2   
   	...
   	//printf("CANPC_set_acceptance_mb3 OK\n");
	...
	//printf("CANPC_set_acceptance2_mb3 OK\n");
...
static void mdlInitializeSizes(SimStruct *S)
{
   	...
   	int_T baud, temp_mb3, presc, sjw, tseg1, tseg2;
	int_T i, id;
	...
	if (baud==8) {
	    ...
	    temp_mb3= 1 + tseg1 + tseg2;
	    if ( temp_mb3 < 8 || temp_mb3 > 25) {
...
#pragma enable_message(1055) 
		...
		if ((ret = INIPC_initialize_board_mb3(&crRessources,(int)mxGetPr(PCI_DEV_ARG)[0])) != 0) {
			printf("-->Error in INIPC_initialize_board_mb3: ");
		...
		//printf("CANPC_enable_dyn_obj_buf_mb3 OK\n");
		...
		//printf("CANPC_initialize_interface_mb3 OK\n");
		...
		for (i=0;i<MAX_OBJECTS;i++) {
			CANAC2_1_sendindex_mb3[i]=-1;
			CANAC2_1_receiveindex_mb3[i]=-1;
			CANAC2_2_sendindex_mb3[i]=-1;
			CANAC2_2_receiveindex_mb3[i]=-1;
		}
		...
		for (i=0;i<mxGetN(CAN1_SEND_ARG);i++) {
			...
			CANAC2_1_sendindex_mb3[i]=msg_id;
			//printf("%d\n",CANAC2_1_sendindex_mb3[i]);
		...
		CANAC2_1_sendmaxindex_mb3=mxGetN(CAN1_SEND_ARG);
		//printf("CAN 1 send: CANPC_define_object_mb3 OK\n");

		for (i=0;i<mxGetN(CAN1_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN1_RECEIVE_ARG)[i];
			CANAC2_1_receiveindex_mb3[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=0;
			} else {
				Type=2;
		...
		CANAC2_1_receivemaxindex_mb3=mxGetN(CAN1_RECEIVE_ARG);
		//printf("CAN 1 receive: CANPC_define_object_mb3 OK\n");

		for (i=0;i<mxGetN(CAN2_SEND_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_SEND_ARG)[i];
			CANAC2_2_sendindex_mb3[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=1;
			} else {
				Type=3;
		...
		CANAC2_2_sendmaxindex_mb3=mxGetN(CAN2_SEND_ARG);
		//printf("CAN 2 send: CANPC_define_object_mb3 OK\n");

		for (i=0;i<mxGetN(CAN2_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_RECEIVE_ARG)[i];
			CANAC2_2_receiveindex_mb3[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=0;
			} else {
				Type=2;
		...
		CANAC2_2_receivemaxindex_mb3=mxGetN(CAN2_RECEIVE_ARG);
		//printf("CAN 2 receive: CANPC_define_object_mb3 OK\n");
		...
		if (CANPC_start_chip_mb3() != 0) {
   		...
   		//printf("CANPC_start_chip_mb3 OK\n");
...
#		ifdef CANAC2_setup_present
		...
		for (i=0;i<CANAC2_init_number;i++) {
			...
			if (CANAC2_init[i].port==1) {
				...
				for (j=0;j<CANAC2_1_sendmaxindex_mb3;j++) {
					if (msg_id==CANAC2_1_sendindex_mb3[j]) { // found
						index=j;
					}
				}
...
#				ifdef DEBUG_CANAC2
					...
					//printf("Index: %d\n",index);
					//printf("Index: %d\n",CANAC2_1_sendindex_mb3[index]);
#				endif

            	frc = CANPC_write_object_mb3(CANAC2_1_send_mb3[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object_mb3: %d\n",frc);
					return;
				} 
			...
			if (CANAC2_init[i].port==2) {
				...
				msg_id=CANAC2_init[i].identifier;
				for (j=0;j<CANAC2_2_sendmaxindex_mb3;j++) {
					if (msg_id==CANAC2_2_sendindex_mb3[j]) { // found
						index=j;
					}
				}
...
#				endif
            	...
            	frc = CANPC_write_object2_mb3(CANAC2_2_send_mb3[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object2_mb3: %d\n",frc);
					return;
				} 
...
static void mdlTerminate(SimStruct *S)
{
	...
	if (CANPC_reinitialize_mb3() != 0) {
   	...
   	//printf("CANPC_reinitialize_mb3 OK\n");
