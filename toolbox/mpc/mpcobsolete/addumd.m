function model=addumd(pmod,dmod)

% ADDUMD Add an unmeasured disturbance model to a plant model.
%        model=addumd(pmod,dmod)
% ADDUMD creates a composite model in which the outputs
% are the sum of the outputs of pmod and dmod.  The inputs
% of the composite model are the manipulated variables from
% pmod, followed by the measured disturbances from pmod (if any),
% followed by the inputs to dmod (which are all classified
% as unmeasured disturbances in the composite model).
%
% Inputs:
...
if nargin ~= 2
   disp('USAGE:  model=addumd(pmod,dmod)')
   return
end
...
elseif any([nym1 nyu1] ~= [nym2 nyu2])
   error('PMOD and DMOD have different number of meas. and/or unmeas. outputs')
elseif nw1 ~= 0
   error('PMOD already contains unmeasured disturbances.')
elseif nd2 ~= 0 | nw2 ~= 0
   error('All inputs to DMOD must be classified as manipulated variables.')
end

[a,b,c,d]=mpcparal(a,b,c,d,ad,bd,cd,dd);
minfom=[T1 n1+n2 nu1 nd1 nu2 nym1 nyu1];
model=ss2mod(a,b,c,d,minfom);

%%% end of function addumd.m %%%