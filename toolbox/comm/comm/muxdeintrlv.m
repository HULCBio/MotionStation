function [deintrlved, state]  = muxdeintrlv(data,delay,varargin)
% MUXDEINTRLV Restore ordering of symbols permuted using a set of shift registers with specified delays.
%  DEINTRLVED = MUXDEINTRLV(DATA, DELAY) restores the elements in DATA by
%  using a set internal shift registers, each with its own delay value.
%  DELAY is a vector whose entries indicate how many symbols each shift
%  register can hold. The length of this vector is the number of shift
%  registers. The resulting deinterleaved data is stored in DEINTRLVED. 
%  If DATA is a matrix, each column is treated as a separate channel and 
%  is processed independently.
%
%  [DEINTRLVED STATE] = MUXDEINTRLV(DATA, DELAY) STATE is a structure that
%  holds the final state of the shift registers. STATE.VALUE stores any
%  unshifted symbols. STATE.INDEX holds the index of the next register to
%  be shifted.
%
...
%  starting from the shift register referenced by INIT_STATE.INDEX.
...
%  $Revision: 1.1.6.4 $ $Date: 2004/04/08 20:48:11 $
...
% --- Error Checking
if nargin > 3
    error('comm:muxdeintrlv:TooManyInp','Too many input arguments.')
end

if (~isnumeric(data) && ~isa(data,'gf'))
    error('comm:muxdeintrlv:DataIsNotNumeric','DATA must be numeric.');
end
...
if isempty(delay)
    error('comm:muxdeintrlv:DelayIsEmpty','DELAY must be a nonempty array of positive integers.');
elseif ~isnumeric(delay)
    error('comm:muxdeintrlv:DelayIsNotAPosInt','DELAY must be a vector of positive integers.');
end

if (del_row > 1 && del_col > 1)
    error('comm:muxdeintrlv:DelayIsMatrix','DELAY must be a vector.');
end

if any([(delay < 0) (delay ~= floor(delay))])
    error('comm:muxdeintrlv:DelayIsNotAPosInt','DELAY must be an vector of positive integers.');
end

% --- Compute DELAY
delayinverse = max(delay)-delay;
delay = delayinverse;
...
if nargin == 3      
    ...
    if ~isstruct(varargin{1})
        error('comm:muxdeintrlv:Init_stateIsNotAStruct','INIT_STATE must be a structure.')
    end
    
    if (length(fieldnames(varargin{1})) < 1 || length(fieldnames(varargin{1})) > 2)
        error('comm:muxdeintrlv:Init_stateTooManyFields','INIT_STATE must be a structure with two fields.');
    end
    
    % --- Check if STATE is valid
    if ~iscell(varargin{1}.value)
        error('comm:muxdeintrlv:Init_stateIsNotACell','INIT_STATE.VALUE must be a cell array.');
    end
    
    % Make sure that the STATE is in correct format.
    if all([~isempty(varargin{1}.value{size(varargin{1}.value,1),1}) (size(varargin{1}.value{size(varargin{1}.value,1),1},2)~=1)])
            error('comm:muxdeintrlv:Init_stateIncorrectFormat', 'INIT_STATE.VALUE must be a cell array of vectors having lengths equal to the corresponding delay values.');
    end
    for idx = 1:size(varargin{1}.value,1)-1
    ...
    for i = 1:length(varargin{1})   
        if (~isnumeric(varargin{1}.value{i}) && ~isa(varargin{1}.value{i},'gf'))
            error('comm:muxdeintrlv:Init_stateIsNotNumeric','INIT_STATE.VALUE must be a cell array of numeric values.');
        end
        
        if ~isnumeric(orig_data)
            if ~(isa(orig_data,'gf') && isa(varargin{1}.value{i},'gf'))
                error('comm:muxdeintrlv:InvalidDataAndInit_state','Both DATA and INIT_STATE.VALUE must be numeric or Galois field array.');                  
    ...
    if (length(varargin{1}.value) > length(delay(:)))
        error('comm:muxdeintrlv:Init_stateTooManyRegs','The number of shift registers must be equal to the length of DELAY.');
    end
    
    if any([(varargin{1}.index < 1) (length(varargin{1}.index) > 1) ~isnumeric(varargin{1}.index)])
        error('comm:muxdeintrlv:IndexIsNotAPosInt','INIT_STATE.INDEX must be a positive scalar integer.');
    end
...
% Returns deinterleaved data and state values
if (~isDataRowVector)
    deintrlved = transpose(final_output); 
else
    deintrlved = final_output;
end
