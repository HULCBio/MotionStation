function [gam_opt,acp,bcp,ccp,dcp,acl,bcl,ccl,dcl] = dhinfopt(varargin)
%DHINFOPT Discrete H-Infinity control synthesis via Gamma iteration.
%
% [GAM_OPT,SS_CP,SS_CL] = DHINFOPT(TSS_,GAMIND,AUX) or
...
%  R. Y. Chiang & M. G. Safonov (11/1994)
...
if max(size(gamind))==0, gamind = 1:size(C1)*[1;0]; end
if max(size(aux))==0, aux = [0.01 1 0]; end
...
while relerr > tol
    ...
    [tacp,tbcp,tccp,tdcp,tacl,tbcl,tccl,tdcl,hinfo] = dhinf(...
