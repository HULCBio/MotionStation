/* 
 * File: rt_lookflatnlincsf.c generated from file
...
real32_T rt_LookFlatNLinCSf(int_T                numDims,
			    const real32_T * u,
			    rt_LUTnWork  * TWork)
{
  ...
  for(k=0; k < numDims; k++) {
    ...
    bpIndex[k] = rt_PLookLinCf(u[k], &bpLambda[k], bpData, maxIndex[k], bpIndex[k]);
  }
                     
  return(rt_IntrpNFlatf(numDims-1, TWork));
}
/* [EOF] rt_lookflatnlincsf.c */
