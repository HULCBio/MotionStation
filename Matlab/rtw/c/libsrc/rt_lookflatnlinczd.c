/* 
 * File: rt_lookflatnlinczd.c generated from file
...
real_T rt_LookFlatNLinCZd(int_T                numDims,
			  const real_T * u,
			  rt_LUTnWork  * TWork)
{
  ...
  for(k=0; k < numDims; k++) {
    ...
    bpIndex[k] = rt_PLookLinCd(u[k], &bpLambda[k], bpData, maxIndex[k], maxIndex[k] >> 1);
  }
                     
  return(rt_IntrpNFlatd(numDims-1, TWork));
}
/* [EOF] rt_lookflatnlinczd.c */
