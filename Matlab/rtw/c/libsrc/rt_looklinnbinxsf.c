/* 
 * File: rt_looklinnbinxsf.c generated from file
...
real32_T rt_LookLinNBinXSf(int_T                numDims,
			   const real32_T * u,
			   rt_LUTnWork  * TWork)
{
  ...
  for(k=0; k < numDims; k++) {
    ...
    bpIndex[k] = rt_PLookBinXf(u[k], &bpLambda[k], bpData, maxIndex[k], bpIndex[k]);
  }
                     
  return(rt_IntrpNLinf(numDims-1, 0, TWork));
}
/* [EOF] rt_looklinnbinxsf.c */
