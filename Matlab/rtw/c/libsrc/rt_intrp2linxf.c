/*
 * File : rt_intrp2linxf.c generated from file
 ...
 *   
 */
real32_T rt_Intrp2LinXf(const int_T          bpIndex0,
			...
			const int_T          maxIndex1)
{
  ...
  real32_T normLambda0 = lambda0;
  real32_T normLambda1 = lambda1;

  if (bpIndex0 < 0) {
    normIdx0 = 0;
    if (!(lambda0 < (real32_T)(bpIndex0))) {
      normLambda0 = (real32_T)bpIndex0;
    }
  } else if (bpIndex0 > (maxIndex0 - 1)) {
    normIdx0 = (maxIndex0-1);
    if (!(lambda0 > (real32_T)(bpIndex0))) {
      normLambda0 = (real32_T)(bpIndex0 - maxIndex0);
    }
  } else {
    normIdx0 = bpIndex0;
  }
  ...
  if (bpIndex1 < 0) {
    normIdx1 = 0;
    if (!(lambda1 < (real32_T)(bpIndex1))) {
      normLambda1 = (real32_T)bpIndex1;
    }
  } else if (bpIndex1 > (maxIndex1 - 1)) {
    normIdx1 = (maxIndex1 - 1);
    if (!(lambda1 > (real32_T)(bpIndex1 - maxIndex1))) {
      normLambda1 = (real32_T)(bpIndex1 - maxIndex1);
    }
  } else {
    normIdx1 = bpIndex1;
  }
  ...
  {
    const real32_T *yupper  = (const real32_T *)&tableData[normIdx0 + numRows*normIdx1];
    const real32_T *ylower  =  yupper + 1;
    const real32_T  yleft   = *yupper + normLambda0 * (*ylower - (*yupper));
