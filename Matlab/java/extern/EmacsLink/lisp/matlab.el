...
(defun matlab-frame-init ()
  ...
  (easy-menu-define
   ...
   '("MATLAB"
     ...
     ["MLint Minor Mode" mlint-minor-mode
      :active (locate-library "mlint")
      :style toggle :selected  mlint-minor-mode
      ]
     ("Auto Fix"
...
(defun matlab-find-other-window-file-line-column (ef el ec &optional debug)
  ...
  (if debug (setq gud-last-frame (cons (buffer-file-name) (string-to-int el))))
...
(defun matlab-find-other-window-via-url (url &optional debug)
  ...
  (if (string-match "^error:\\(.*\\),\\([0-9]+\\),\\([0-9]+\\)$" url)
