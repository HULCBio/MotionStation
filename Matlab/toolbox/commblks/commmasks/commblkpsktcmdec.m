function [errStr, iconStr, cplxconstpts, t] = commblkpsktcmdec(block,action)
% COMMBLKPSKTCMDEC Mask function for M-PSK TCM Decoder block
% 
% Usage:
%      commblkpsktcmdec(block,'action');
%      where action corresponds to one of the following callbacks:
%      cbOpMode or init.

% Copyright 1996-2003 The MathWorks, Inc.
% $Revision: 1.1.6.2 $ $Date: 2003/07/30 02:48:30 $

% Default fcn: COMMBLKDEF_PSKTCMDEC
...
if(~isok)
    ...
    if(maskTrellis.numOutputSymbols~=Mnum)
        ...
        [cplxconstpts err] = tcmconstmapper(maskTrellis, Mnum, 'psk');
    end  
end
...
if isempty(maskTbdepth) || ~isscalar(maskTbdepth) ||  ...
    ...
    s.mmi = 'commblks:commblkpsktcmdec:tbDepthCheck';
end
