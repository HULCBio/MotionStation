/* $Revision: 1.5 $ $Date: 2002/03/25 04:02:45 $*/
/* canac2setupe.c - xPC Target, non-inlined S-function driver for CAN-AC2-ISA (extended) from Softing (single board)  */
...
#define S_FUNCTION_NAME canac2setupe
...
static void mdlInitializeSizes(SimStruct *S)
{
   	...
   	int_T baud, temp, presc, sjw, tseg1, tseg2;
	int_T i, id;
...
#include "time_xpcimport.c"
	...
	for (i=0;i<mxGetN(CAN1_SEND_ARG);i++) {
		...
		if (id >= 0) { // extended identifiers  
			if (id > MAX_IDS_EXT) {
				sprintf(msg,"CAN1 send: the extended identifiers must be in the range of 0..536870911\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		    ...
		    id= -id;
			if (id > MAX_IDS + 1) {
				sprintf(msg,"CAN1 send: the standard identifiers must be in the range of -1..-2032\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		}
	...
	for (i=0;i<mxGetN(CAN1_RECEIVE_ARG);i++) {
		...
		if (id >= 0) { // extended identifiers  
			if (id > MAX_IDS_EXT) {
				sprintf(msg,"CAN1 receive: the extended identifiers must be in the range of 0..536870911\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		} else { // standard identifiers
		    id= -id;
			if (id > MAX_IDS + 1) {
				sprintf(msg,"CAN1 receive: the standard identifiers must be in the range of -1..-2032\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		}
	...
	for (i=0;i<mxGetN(CAN2_SEND_ARG);i++) {
		...
		if (id >= 0) { // extended identifiers  
			if (id > MAX_IDS_EXT) {
				sprintf(msg,"CAN2 send: the extended identifiers must be in the range of 0..536870911\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		} else { // standard identifiers
		    id= -id;
			if (id > MAX_IDS + 1) {
				sprintf(msg,"CAN2 send: the standard identifiers must be in the range of -1..-2032\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		}
	...
	for (i=0;i<mxGetN(CAN2_RECEIVE_ARG);i++) {
		...
		if (id >= 0) { // extended identifiers  
			if (id > MAX_IDS_EXT) {
				sprintf(msg,"CAN2 receive: the extended identifiers must be in the range of 0..536870911\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		} else { // standard identifiers
		    id= -id;
			if (id > MAX_IDS + 1) {
				sprintf(msg,"CAN2 receive: the standard identifiers must be in the range of -1..-2032\n");
        		ssSetErrorStatus(S,msg);
        		return;
			}
		}
...
#pragma enable_message(1055) 
		...
		for (i=0;i<mxGetN(CAN1_SEND_ARG);i++) {
			...
			if (msg_id >= 0) {  // extended identifiers
				Type=3;
			} else {
				Type=1;
				msg_id= -msg_id -1 ;
