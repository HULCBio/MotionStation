/* $Revision: 1.6 $ $Date: 2002/03/25 04:03:30 $ */
/* canac2pcisetup_mb1.c - xPC Target, non-inlined S-function driver for CAN-AC2-PCI from Softing (multi board)  */
...
#define S_FUNCTION_NAME canac2pcisetup_mb1
...
extern int CANAC2_FIRMWARE_LOADED_mb1;
...
extern int CANAC2_1_receivemaxindex_mb1;
extern int CANAC2_1_receiveindex_mb1[MAX_OBJECTS];
extern int CANAC2_2_sendmaxindex_mb1;
extern int CANAC2_2_sendindex_mb1[MAX_OBJECTS];
extern int CANAC2_2_receivemaxindex_mb1;
extern int CANAC2_2_receiveindex_mb1[MAX_OBJECTS];
...
// Routine to evaluate error codes of CANPC_reset_board_mb1
...
static int Initialize_can_parameter(SimStruct *S)
{
   ...
   //printf("CANPC_reset_chip_mb1 OK\n\n");

   // Display SW and HW version, chip type and licence (optionally)
   if (CANPC_get_version_mb1(&sw_version,&fw_version,&hw_version,&license,can_chip_type) != 0) {
      printf("-->Error in CANPC_get_version_mb1 \n\n\n");
      return(-1);
   } else {
   ...
   // Set Bit timing for CAN 1 and 2
   	...
   	//printf("CANPC_initialize_chip_mb1 OK\n");
	...
	baud=(int)mxGetPr(CAN2_BAUDRATE_ARG)[0];
   	...
   	//printf("CANPC_initialize_chip2_mb1 OK\n");
	...
	// Set acceptance filter for CAN 1 and 2   
   	...
   	//printf("CANPC_set_acceptance_mb1 OK\n");

   	if (CANPC_set_acceptance2_mb1(ACCEPT_MASK_2, ACCEPT_CODE_2, ACCEPT_MASK_XTD_2, ACCEPT_CODE_XTD_2) != 0) {
      	printf("-->Error in CANPC_set_acceptance2_mb1 \n");
      	return(-1);
    }
	//printf("CANPC_set_acceptance2_mb1 OK\n");
   	...
   	//printf("CANPC_set_mode_mb1 OK\n");

   	if (CANPC_set_mode2_mb1(SLEEPMODE_2, SPEEDMODE_2) != 0) {
      	printf("-->Error in CANPC_set_mode2_mb1 \n");
      	return(-1);
    }
	//printf("CANPC_set_mode2_mb1 OK\n");
	...
	//printf("CANPC_set_output_control_mb1 OK\n");

   	if (CANPC_set_output_control2_mb1(OUTPUT_CONTROL_2) != 0) {
      	printf("-->Error in CANPC_set_output_control2_mb1 \n");
      	return(-1);
    }
	//printf("CANPC_set_output_control2_mb1 OK\n");
...
static void mdlInitializeSizes(SimStruct *S)
{
   	...
   	int_T baud, temp_mb1, presc, sjw, tseg1, tseg2;
	int_T i, id;
	...
	if (baud==8) {
	    ...
	    temp_mb1= 1 + tseg1 + tseg2;
	    if ( temp_mb1 < 8 || temp_mb1 > 25) {
...
#pragma enable_message(1055) 
		...
		if ((ret = INIPC_initialize_board_mb1(&crRessources,(int)mxGetPr(PCI_DEV_ARG)[0])) != 0) {
			printf("-->Error in INIPC_initialize_board_mb1: ");
			printInitErrorText(ret); 
			return;
		}

        if (!CANAC2_FIRMWARE_LOADED_mb1) {
		   	// Load firmware
		...
		//printf("CANPC_enable_dyn_obj_buf_mb1 OK\n");
		...
		//printf("CANPC_initialize_interface_mb1 OK\n");
		...
		for (i=0;i<MAX_OBJECTS;i++) {
			CANAC2_1_sendindex_mb1[i]=-1;
			CANAC2_1_receiveindex_mb1[i]=-1;
			CANAC2_2_sendindex_mb1[i]=-1;
			CANAC2_2_receiveindex_mb1[i]=-1;
		}
		...
		for (i=0;i<mxGetN(CAN1_SEND_ARG);i++) {
			...
			CANAC2_1_sendindex_mb1[i]=msg_id;
			//printf("%d\n",CANAC2_1_sendindex_mb1[i]);
		...
		CANAC2_1_sendmaxindex_mb1=mxGetN(CAN1_SEND_ARG);
		//printf("CAN 1 send: CANPC_define_object_mb1 OK\n");

		for (i=0;i<mxGetN(CAN1_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN1_RECEIVE_ARG)[i];
			CANAC2_1_receiveindex_mb1[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=0;
			} else {
				Type=2;
		...
		CANAC2_1_receivemaxindex_mb1=mxGetN(CAN1_RECEIVE_ARG);
		//printf("CAN 1 receive: CANPC_define_object_mb1 OK\n");

		for (i=0;i<mxGetN(CAN2_SEND_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_SEND_ARG)[i];
			CANAC2_2_sendindex_mb1[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=1;
			} else {
				Type=3;
			...
			if (CANPC_define_object2_mb1(msg_id,
	                                ...
	                                AutoRemoteEnable,
	                                TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object_mb1 for send objects CAN 2\n");
	            return;
	      	}
	  	}
		CANAC2_2_sendmaxindex_mb1=mxGetN(CAN2_SEND_ARG);
		//printf("CAN 2 send: CANPC_define_object_mb1 OK\n");

		for (i=0;i<mxGetN(CAN2_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_RECEIVE_ARG)[i];
			CANAC2_2_receiveindex_mb1[i]=msg_id;
			if (msg_id >= 0) {  // standard identifiers
				Type=0;
			} else {
				Type=2;
			...
			if (CANPC_define_object2_mb1(msg_id,
	                                &CANAC2_2_receive_mb1[i],
	                                Type,
	                                ReceiveIntEnable,
	                                AutoRemoteEnable,
	                                TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object_mb1 for receive objects CAN 2\n");
	            return;
	      	}
	  	}
		CANAC2_2_receivemaxindex_mb1=mxGetN(CAN2_RECEIVE_ARG);
		//printf("CAN 2 receive: CANPC_define_object_mb1 OK\n");
		...
		if (CANPC_start_chip_mb1() != 0) {
   		...
   		//printf("CANPC_start_chip_mb1 OK\n");
...
#				ifdef DEBUG_CANAC2
					...
					//printf("Index: %d\n",index);
					//printf("Index: %d\n",CANAC2_1_sendindex_mb1[index]);
#				endif

            	frc = CANPC_write_object_mb1(CANAC2_1_send_mb1[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object_mb1: %d\n",frc);
					return;
				} 
...
#				endif
            	...
            	frc = CANPC_write_object2_mb1(CANAC2_2_send_mb1[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object2_mb1: %d\n",frc);
					return;
				} 
...
static void mdlTerminate(SimStruct *S)
{
	...
	if (CANPC_reinitialize_mb1() != 0) {
   	...
   	//printf("CANPC_reinitialize_mb1 OK\n");
