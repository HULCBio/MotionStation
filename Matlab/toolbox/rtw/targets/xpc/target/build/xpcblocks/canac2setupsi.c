/* $Revision: 1.6 $ $Date: 2002/03/25 04:02:51 $*/
/* canac2setupi.c - xPC Target, non-inlined S-function driver for CAN-AC2-ISA (standard and interrupt driven) from Softing (single board)  */
...
#define S_FUNCTION_NAME canac2setupsi
...
#define CAN1_BAUDRATE_ARG           ssGetSFcnParam(S,1)
#define CAN1_USER_BAUDRATE_ARG      ssGetSFcnParam(S,2)
#define CAN2_BAUDRATE_ARG           ssGetSFcnParam(S,3)
#define CAN2_USER_BAUDRATE_ARG      ssGetSFcnParam(S,4)
#define CAN1_SEND_ARG               ssGetSFcnParam(S,5)
#define CAN1_RECEIVE_ARG            ssGetSFcnParam(S,6)
#define CAN2_SEND_ARG               ssGetSFcnParam(S,7)
#define CAN2_RECEIVE_ARG            ssGetSFcnParam(S,8)
#define INT_PORT_ARG            	ssGetSFcnParam(S,9)
#define INT_MSGIND_ARG            	ssGetSFcnParam(S,10)
...
extern int CANAC2_FIRMWARE_LOADED;
...
extern int CANAC2_1_receivemaxindex;
extern int CANAC2_1_receiveindex[MAX_OBJECTS];
extern int CANAC2_2_sendmaxindex;
extern int CANAC2_2_sendindex[MAX_OBJECTS];
extern int CANAC2_2_receivemaxindex;
extern int CANAC2_2_receiveindex[MAX_OBJECTS];
...
void printInitErrorText(int iErrorCode)
{
...
void printResetErrorText(int iErrorCode)
{
...
int  Initialize_can_parameter(SimStruct *S)
{
	int sw_version;
	int fw_version;
	int hw_version;
   ...
   if (CANPC_reset_chip() != 0) {
      printf("-->Error in CANPC_reset_chip \n");
      return(-1);
   }
   //printf("CANPC_reset_chip OK\n\n");
   ...
   // Set Bit timing for CAN 1 and 2
   	...
   	if (CANPC_initialize_chip(presc, sjw, tseg1, tseg2, 0) != 0) {
      	printf("-->Error in CANPC_initialize_chip \n");
      	return(-1);
    }
   	//printf("CANPC_initialize_chip OK\n");
	...
	baud=(int)mxGetPr(CAN2_BAUDRATE_ARG)[0];
   	...
   	if (CANPC_initialize_chip2(presc, sjw, tseg1, tseg2, 0) != 0) {
      	printf("-->Error in CANPC_initialize_chip2 \n");
      	return(-1);
    }
   	//printf("CANPC_initialize_chip2 OK\n");
	...
	// Set acceptance filter for CAN 1 and 2   
   	...
   	//printf("CANPC_set_acceptance OK\n");
...
static void mdlInitializeSizes(SimStruct *S)
{
   	...
   	int_T baud, temp, presc, sjw, tseg1, tseg2, port, int_msgind;
	int_T i, id;
...
#include "time_xpcimport.c"
    ...
    if ( mxGetM(INT_MSGIND_ARG)!=1 &&  mxGetN(INT_MSGIND_ARG)!=1) {
    	sprintf(msg,"argument for interrupt message Id index must be a scalar\n");
        ssSetErrorStatus(S,msg);
        return;
  	}
	port=(int_T)mxGetPr(INT_PORT_ARG)[0];
	int_msgind=(int_T)mxGetPr(INT_MSGIND_ARG)[0];
	if (port==2 || port==3) {
		if (int_msgind==0) {
			sprintf(msg,"argument interrupt message Id index is invalid: value must be higher than 0\n");
        	ssSetErrorStatus(S,msg);
       	 	return;
		}
	}
	if (port==2) {		// CAN1
		if (mxGetN(CAN1_RECEIVE_ARG)<int_msgind ) {
		   	sprintf(msg,"argument interrupt message Id index points to a non-existing identifier for CAN1\n");
        	ssSetErrorStatus(S,msg);
       	 	return;
		}
	} else if (port==3) {	// CAN2
		if (mxGetN(CAN2_RECEIVE_ARG)<int_msgind ) {
		   	sprintf(msg,"argument interrupt message Id index points to a non-existing identifier for CAN2\n");
        	ssSetErrorStatus(S,msg);
       	 	return;
		}
	}
...
#pragma enable_message(1055) 
        ...
        if (!CANAC2_FIRMWARE_LOADED) {
		   	// Load firmware
		   	printf("        downloading CAN-AC2 firmware...\n");
			...
			printf("        downloading CAN-AC2 firmware finished\n");
		    CANAC2_FIRMWARE_LOADED=1;
	    }
		...
		if (CANPC_enable_dyn_obj_buf() != 0) {
      		printf("-->Error in CANPC_enable_dyn_obj_buf \n");
      		return;
   		}
		//printf("CANPC_enable_dyn_obj_buf OK\n");
	    ...
	    if (CANPC_initialize_interface(ReceiveFifoEnable,
	                                   ReceivePollAll,
	                                   ReceiveEnableAll,
	                                   ReceiveIntEnableAll,
	                                   AutoRemoteEnableAll,
	    	...
	    	printf("-->Error in CANPC_initialize_interface \n");
	        return;
	    }
		//printf("CANPC_initialize_interface OK\n");
		...
		for (i=0;i<MAX_OBJECTS;i++) {
			CANAC2_1_sendindex[i]=-1;
			CANAC2_1_receiveindex[i]=-1;
			CANAC2_2_sendindex[i]=-1;
			CANAC2_2_receiveindex[i]=-1;
		}
	    ...
	    for (i=0;i<mxGetN(CAN1_SEND_ARG);i++) {
			...
			CANAC2_1_sendindex[i]=msg_id;
			if (CANPC_define_object(msg_id,
	                                &CANAC2_1_send[i],
	                                Type,
	                                ReceiveIntEnable,
	                                AutoRemoteEnable,
	                                TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object for send objects CAN 1\n");
	            return;
	      	}
	  	}

		CANAC2_1_sendmaxindex=mxGetN(CAN1_SEND_ARG);
		//printf("CAN 1 send: CANPC_define_object OK\n");

        Type = 0; // receive CAN1	   
	    for (i=0;i<mxGetN(CAN1_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN1_RECEIVE_ARG)[i];
			CANAC2_1_receiveindex[i]=msg_id;
			if ((int_T)mxGetPr(INT_PORT_ARG)[0]==2) {
				if (((int_T)mxGetPr(INT_MSGIND_ARG)[0]-1) == i) {
					ReceiveIntEnable=1;
				}
			}
			if (CANPC_define_object(msg_id,
	                            ...
	                            AutoRemoteEnable,
	                            TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object for receive objects CAN 1\n");
	            return;
	      	}
			ReceiveIntEnable=0;
	  	}
		CANAC2_1_receivemaxindex=mxGetN(CAN1_RECEIVE_ARG);
		//printf("CAN 1 receive: CANPC_define_object OK\n");

		Type = 1; // send CAN2	   
	    for (i=0;i<mxGetN(CAN2_SEND_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_SEND_ARG)[i];
			CANAC2_2_sendindex[i]=msg_id;
			if (CANPC_define_object2(msg_id,
	                                &CANAC2_2_send[i],
	                                Type,
	                                ReceiveIntEnable,
	                                AutoRemoteEnable,
	                                TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object for send objects CAN 2\n");
	            return;
	      	}
	  	}
		CANAC2_2_sendmaxindex=mxGetN(CAN2_SEND_ARG);
		//printf("CAN 2 send: CANPC_define_object OK\n");

        Type = 0; // receive CAN2
	    for (i=0;i<mxGetN(CAN2_RECEIVE_ARG);i++) {
	   		msg_id=(int_T)mxGetPr(CAN2_RECEIVE_ARG)[i];
			CANAC2_2_receiveindex[i]=msg_id;
			if ((int_T)mxGetPr(INT_PORT_ARG)[0]==3) {
				if (((int_T)mxGetPr(INT_MSGIND_ARG)[0]-1) == i) {
					ReceiveIntEnable=1;
				}
			}
			if (CANPC_define_object2(msg_id,
	                                ...
	                                AutoRemoteEnable,
	                                TransmitAckEnable) != 0) {
	       		printf("-->Error in CANPC_define_object for receive objects CAN 2\n");
	            return;
	      	}
			ReceiveIntEnable=0;
	  	}
		CANAC2_2_receivemaxindex=mxGetN(CAN2_RECEIVE_ARG);
		//printf("CAN 2 receive: CANPC_define_object OK\n");
...
#				ifdef DEBUG_CANAC2
					...
					//printf("Index: %d\n",index);
					//printf("Index: %d\n",CANAC2_1_sendindex[index]);
#				endif

            	frc = CANPC_write_object(CANAC2_1_send[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object: %d\n",frc);
					return;
				} 

            	//wait specified ms
				rl32eWaitDouble(CANAC2_init[i].wait_ms/1000.0);
...
#				endif
            	...
            	frc = CANPC_write_object2(CANAC2_2_send[index], CANAC2_init[i].no_bytes, CANAC2_init[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object2: %d\n",frc);
					return;
				} 
...
#		ifdef CANAC2_setup_present
		...
		for (i=0;i<CANAC2_term_number;i++) {

			if (CANAC2_term[i].port==1) {
			
				msg_id=CANAC2_term[i].identifier;
				for (j=0;j<CANAC2_1_sendmaxindex;j++) {
					if (msg_id==CANAC2_1_sendindex[j]) { // found
						index=j;
					}
...
#				ifdef DEBUG_CANAC2
		    		printf("Port: %d, Id: %d, Data: %x %x %x %x %x %x %x %x, NoBytes: %d, Wait: %d\n",
						CANAC2_term[i].port,
						CANAC2_term[i].identifier,
						CANAC2_term[i].data[0],
						CANAC2_term[i].data[1],
						CANAC2_term[i].data[2],
						CANAC2_term[i].data[3],
						CANAC2_term[i].data[4],
						CANAC2_term[i].data[5],
						CANAC2_term[i].data[6],
						CANAC2_term[i].data[7],
						CANAC2_term[i].no_bytes,
						CANAC2_term[i].wait_ms);
					//printf("Index: %d\n",index);
					//printf("Index: %d\n",CANAC2_1_sendindex[index]);
#				endif

            	frc = CANPC_write_object(CANAC2_1_send[index], CANAC2_term[i].no_bytes, CANAC2_term[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object: %d\n",frc);
					return;
				} 
            	...
            	//wait specified ms
				rl32eWaitDouble(CANAC2_term[i].wait_ms/1000.0);


         	}

			if (CANAC2_term[i].port==2) {
...
#				endif
            	...
            	frc = CANPC_write_object2(CANAC2_2_send[index], CANAC2_term[i].no_bytes, CANAC2_term[i].data);
				if (frc<0) {
					printf("ERROR:  CANPC_write_object2: %d\n",frc);
					return;
				} 
	...
	if (CANPC_reinitialize() != 0) {
    	printf("-->Error in CANPC_start_chip \n");
		return;
	}
   	//printf("CANPC_reinitialize OK\n");
