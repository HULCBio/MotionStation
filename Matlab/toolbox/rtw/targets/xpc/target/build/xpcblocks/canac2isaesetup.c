/* $Revision: 1.5 $ $Date: 2002/03/25 04:02:06 $ */
...
#define 	S_FUNCTION_NAME 	canac2isaesetup
...
static int  Initialize_can_parameter(SimStruct *S)
{
	...
	// Set acceptance filter for CAN 1 and 2   
   	...
   	if (CANPC_set_acceptance2(ACCEPT_MASK_1, ACCEPT_CODE_1, ACCEPT_MASK_XTD_1, ACCEPT_CODE_XTD_1) != 0) {
	...
	if (CANPC_set_mode2(SLEEPMODE_1, SPEEDMODE_1) != 0) {
   	...
   	if (CANPC_set_output_control2(OUTPUT_CONTROL_1) != 0) {
...
static void mdlStart(SimStruct *S)
{
	...
	crRessources.ulDPRMemBase=((int)mxGetPr(MEM_BASE_ADDRESS_ARG)[0])-1;
	...
	if (!CANAC2_FIRMWARE_LOADED[(int_T)mxGetPr(BOARD_ARG)[0]-1]) {
		...
		printf("        downloading CAN-AC2-ISA E firmware\n");
		if ((ret = CANPC_reset_board()) != 0) {
   			printf("-->Error in CANPC_reset_board: ");
   			printResetErrorText(ret);
   			return;
		}
		printf("        downloading CAN-AC2-ISA E firmware finished\n");
	...
	for (i=0;i<mxGetN(CAN2_SEND_ARG);i++) {
		...
		if (CANPC_define_object2(msg_id,
	                            ...
	                            AutoRemoteEnable,
	                            TransmitAckEnable) != 0) {
	    	printf("-->Error in CANPC_define_object2 for send objects CAN 2\n");
	        return;
	    }
		if (index!=test) {
			ssSetErrorStatus(S,"indexing assumption failed");
	...
	for (i=0;i<mxGetN(CAN2_RECEIVE_ARG);i++) {
		...
		if (CANPC_define_object2(msg_id,
	                            ...
	                            AutoRemoteEnable,
	                            TransmitAckEnable) != 0) {
	    	printf("-->Error in CANPC_define_object2 for receive objects CAN 2\n");
	        return;
	    }
		if (index!=test) {
			ssSetErrorStatus(S,"indexing assumption failed");
		...
		if ((int_T)mxGetPr(CAN_INIT_ARG)[0]) {
			...
			for (i=0;i<(int_T)mxGetPr(CAN_INIT_ARG)[0];i++) {
				...
				if (((int_T)mxGetPr(CAN_INIT_ARG)[start])==1) {  //CAN port 1
					frc = CANPC_write_object((int_T)mxGetPr(CAN_INIT_ARG)[start+2], (int_T)mxGetPr(CAN_INIT_ARG)[start+3], data);
					if (frc<0) {
						printf("ERROR:  CANPC_write_object CAN 1: %d\n",frc);
						return;
					} 
				} else {
					frc = CANPC_write_object2((int_T)mxGetPr(CAN_INIT_ARG)[start+2], (int_T)mxGetPr(CAN_INIT_ARG)[start+3], data);
					if (frc<0) {
						printf("ERROR:  CANPC_write_object CAN 2: %d\n",frc);
						return;
					} 
				}
		...
		if ((int_T)mxGetPr(CAN_TERM_ARG)[0]) {
			...
			for (i=0;i<(int_T)mxGetPr(CAN_TERM_ARG)[0];i++) {
				...
				if (((int_T)mxGetPr(CAN_TERM_ARG)[start])==1) {  //CAN port 1
		 			frc = CANPC_write_object((int_T)mxGetPr(CAN_TERM_ARG)[start+2], (int_T)mxGetPr(CAN_TERM_ARG)[start+3], data);
					if (frc<0) {
						printf("ERROR:  CANPC_write_object CAN 1: %d\n",frc);
						return;
					} 
				} else {
					frc = CANPC_write_object2((int_T)mxGetPr(CAN_TERM_ARG)[start+2], (int_T)mxGetPr(CAN_TERM_ARG)[start+3], data);
					if (frc<0) {
						printf("ERROR:  CANPC_write_object CAN 2: %d\n",frc);
						return;
					} 
