/* $Revision: 1.4 $ $Date: 2002/03/25 04:03:18 $ */
/* canac2pcisend_mb3.c - xPC Target, non-inlined S-function driver for CAN-AC2-PCI from Softing (multi board)  */
...
#define S_FUNCTION_NAME canac2pcisend_mb3
...
//starttime=rl32eGetTicksDouble();
    		frc = CANPC_write_object_mb3(CANAC2_1_send_mb3[index],DataLength, p_data);
//printf("%f\n",rl32eETimeDouble(rl32eGetTicksDouble(),starttime));
		...
		for (i=0;i<mxGetN(CAN_ID_ARG);i++) {
    		...
    		DataLength = 8;
    		frc = CANPC_write_object2_mb3(CANAC2_2_send_mb3[index],DataLength, p_data);
			if (frc) {
