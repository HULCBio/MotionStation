/* $Revision: 1.4 $ $Date: 2002/03/25 04:04:54 $*/
/* canac2receives.c - xPC Target, non-inlined S-function driver for CAN-AC2-ISA (standard) from Softing (single board)  */
...
#define S_FUNCTION_NAME canac2receives
...
{        
	...
	if (port==1) {		// CAN 1
    	for (i=0;i<mxGetN(CAN_ID_ARG);i++) {
			...
			index=-1;
			for (j=0;j<CANAC2_1_receivemaxindex;j++) {
				if (msg_id==CANAC2_1_receiveindex[j]) { // found
					index=j;
				}
			}
			if (index==-1) {
            ...
            frc = CANPC_read_rcv_data(CANAC2_1_receive[index], p_data, &Time);
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data: %d\n",frc);
				return;
			} 
    	...
    	for (i=0;i<mxGetN(CAN_ID_ARG);i++) {
           	msg_id=(int)mxGetPr(CAN_ID_ARG)[i];
			index=-1;
			for (j=0;j<CANAC2_2_receivemaxindex;j++) {
				if (msg_id==CANAC2_2_receiveindex[j]) { // found
					index=j;
				}
			}
			if (index==-1) {
            ...
            frc = CANPC_read_rcv_data2(CANAC2_2_receive[index], p_data, &Time);
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data2: %d\n",frc);
				return;
			} 
            value=(double *) &p_data[0];
			y[i]=*value;
