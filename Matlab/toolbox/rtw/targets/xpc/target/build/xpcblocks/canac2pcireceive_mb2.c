/* $Revision: 1.4 $ $Date: 2002/03/25 04:02:24 $ */
/* canac2pcireceive_mb2.c - xPC Target, non-inlined S-function driver for CAN-AC2-PCI from Softing (multi board)  */
...
#define S_FUNCTION_NAME canac2pcireceive_mb2
...
{        
	...
	if (port==1) {		// CAN 1
    	for (i=0;i<mxGetN(CAN_ID_ARG);i++) {
			...
			index=-1;
			for (j=0;j<CANAC2_1_receivemaxindex_mb2;j++) {
				if (msg_id==CANAC2_1_receiveindex_mb2[j]) { // found
					index=j;
				}
			}
			if (index==-1) {
            ...
            frc = CANPC_read_rcv_data_mb2(CANAC2_1_receive_mb2[index], p_data, &Time);
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data_mb2: %d\n",frc);
				return;
			} 
    	...
    	for (i=0;i<mxGetN(CAN_ID_ARG);i++) {
           	msg_id=(int)mxGetPr(CAN_ID_ARG)[i];
			index=-1;
			for (j=0;j<CANAC2_2_receivemaxindex_mb2;j++) {
				if (msg_id==CANAC2_2_receiveindex_mb2[j]) { // found
					index=j;
				}
			}
			if (index==-1) {
            ...
            frc = CANPC_read_rcv_data2_mb2(CANAC2_2_receive_mb2[index], p_data, &Time);
			if (frc<-1) {
				printf("ERROR:  CANPC_read_rcv_data2_mb2: %d\n",frc);
				return;
			} 
            value=(double *) &p_data[0];
			y[i]=*value;
