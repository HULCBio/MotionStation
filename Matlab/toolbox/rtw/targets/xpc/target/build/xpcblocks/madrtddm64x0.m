function [channelout, couplingout, rangeout, gainout, maskdisplay, maskdescription]= madrtddm64x0(phase, boardType, channel, coupling, range, gain)

% MADRTDDM64x0 - Mask Initialization for RTD DM64x0 A/D section

% Copyright 1996-2004 The MathWorks, Inc.
% $Revision: 1.4.6.1 $ $Date: 2004/03/02 03:04:06 $
  ...
  if phase == 1
    ...
    if length(blocks)>1
      ...
      baseAddrs = get_param(blocks, 'baseAddr');
  ...
  if phase == 2
    ...
    % check for coupling elements smaller than 1 or greater 2
    if ~isempty(find(coupling<1)) | ~isempty(find(coupling>2))
      error('Coupling Vector elements must be either 1 (SE) or 2 (DIFF)');
    end
    ...
    if ~isempty(find(ismember(gain,[1,2,4,8])==0))
      error('Gain Vector elements must be either 1, 2, 4, or 8');
    end
