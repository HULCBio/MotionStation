...
if narg < arg % legend
    ...
    if ~isempty(msg), warning(msg); end
    return;
...
while n <= nargs
  ...
  elseif iscell(argin{n}) 
    ...
    if ~iscellstr(argin{n})
      error('cell array argument must be a cell array of strings');
    end
    strings = argin{n};
...
if isprop(handle(a),'shapetype')
    if strcmpi(get(handle(a),'shapetype'),'legend') || ...
        ...
        error('plot axes can not be a legend or colorbar');
    end
...
if mod(n,2)==0 % must be even
    ...
    for i=1:2:n
        if ~ischar(args{i})
            error('??non string property name')
        elseif ~isprop('scribe','legend',args{i})
            error(sprintf('unknown property %s',args{i}));
        end
    end
else
    error('invalid arguments - must be an even number of property/value pairs');
end
...
if isempty(str)
  ...
  if length(str) ~= length(ch)
    if ~auto_children || length(str) > length(ch)
      msg = 'Ignoring extra legend entries.';
    end
