...
1/9/96   vk.. gethexnumber(), getshorthex() added
2/16/96  vk..gethexnumber() changed to return ESC_SEL, when user hits
         Carraige Return on nothing.
...
/*
    gethexnumber    -   Get Number in hexadecimal format

    Prototype in:       menu.h

    Parameters Passed:  void

    Return Value:       hexansw     -   integer corresponding to the number
                                        input by the user or ESC_SEL if the
                                        user hit the ESC key or Cariage Return

    Remarks:            the routine allows the user to input hex numbers 0-9,
                        a - f (A - F) and terminate with a CR.  If invalid keys are
                        depressed the BEL sound is echoed.
*/
int gethexnumber()
{
    ...
    /*
        Get a hexdecimal number from the console
    */
    while (TRUE)
    {
        fflush(stdout);
        ...
        */
        ...
        /*
            Numbers
        */
        if ( ((chr >= 0x30) && (chr <= 0x39)) || ((chr >= 0x41) && (chr <= 0x46)) || ((chr >= 0x61) && (chr <= 0x66)) )
        {
            putchar(chr);                       /* echo character */
            *answbufptr++ = chr;                /* store character */
            continue;
        ...
        */
        ...
        else if (chr != CR )                    /* allow Carriage return on nothing */
        {
            putchar(BEL);
            continue;
        }
        ...
        */
        if (chr == CR)
        {
            *answbufptr = 0;                    /* terminate string */
            ...
            intanswer = (int)(strtol(answbuf,0,16));
            break;
        }
...
/*
    getshorthex      -  Get Unsigned Short Number in HEX format
                        with Text Prompt
    ...
    Remarks:
*/
short getshorthex(prompt,min,max)
char * prompt;
short min;
short max;
{
