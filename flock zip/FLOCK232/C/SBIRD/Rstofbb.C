...
2/1/96   vk..getmultirecord(),displaymultirecord() changed. If ISA interface
         THEN treat button value as a WORD not a byte.
2/12/96  vk..displaymultirecord() changed. If ISA interface
         THEN treat address as a WORD not a byte when in group mode
2/13/96  vk..getmultibirddata() changed. IF ISA interface then to
         stop streaming call bird_clearoutputbuffer_isa() function.
2/16/96  vk..getmultibirddata() changed. IF ISA interface then before
         sending stream cmd call bird_clearoutputbuffer_isa() function.
         Done to get rid of Data Ready Character sitting in the
         buffer if Data Ready Character Output was enabled.
  ...
  while (TRUE)
  {
    ...
    */
...
#endif
	  send_cmd((unsigned char *)"@",1);     /* START streaming */
	  firsttime = FALSE;
	}
        ...
        if (ckkbhit())
	{
	  clearkey();
	    ...
	    printf("*NOTE* %d Phase Errors have occurred\n\r",phaseerror_count);
          SETTEXTPOS(1,numfbbrcvrs+4);
	  hitkeycontinue();
	  return(TRUE);
	}
    ...
    */
    if (getmultirecords(outputmode, buttonmode, datamode, startaddr, stopaddr, birddata))
      displaymultidata(datamode, buttonmode, displayon, startaddr,stopaddr, birddata, datafilestream);
    else
      return(FALSE);
  }
}
...
unsigned char * birddata;
{
  ...
  */
  if (!fbbgroupdataflg)
  {
    ...
    */ 
    /* we must send the 'B' command to the master */
    if (hostSyncFlag)
      startaddr = 1;
    ...
    {  
      ...
      EXCEPT FOR DEVICE AT FBB ADDRESS 1 when in HostSync mode
      (we need to send a 'B' command to the master to trigger
      a measurement cycle )
      */           
      if((addr==1) && (hostSyncFlag))
      {
        /* do not skip sending a 'B' command to the master */
      }
      else if(!(fbbsystemstatus[addr] & 0x20))
	continue;
	...
	else
	{
	  ...
	  if (interface == ISA_BUS)  /* IF ISA interface THEN*/
	    birddata += buttonmode; /* button value ias a word,
	                                          so inc more */
	  /*
	  Save the Button Value for Each Address
	  */
	  multibuttonvalue[addr] = (unsigned char)(buttonvalue);
	}
    }
  } 
  else /* The Bird is in the Group Data Mode */
  {
    ...
    */
    ...
    {
      if (!readbirddata((short *)birddata, datasize, outputmode, buttonmode))
      {
	...
	*/
	birddata += (datasize * 2) + 1 + buttonmode;
        #ifdef DOS
          if (interface == ISA_BUS)  /* IF ISA interface THEN*/
            birddata += buttonmode; /* button value is a word, so inc more */
        #endif

	/*
        Save the Button Value for Each Address
	*/
	multibuttonvalue[numrecords] = (unsigned char)(buttonvalue);
      }
    }
  }
...
{
    unsigned char tempstartaddr;
    unsigned char tempstopaddr;
...
//printf("Data Format command sent to bird %d...\n\r", addr);
//hitkeycontinue();
	    } 
	    
//printf("All assigned birds data format set up...\n\r");
//hitkeycontinue();   
...
FILE * datafilestream;
{
	...
	*/
    ...
    if (interface == ISA_BUS)  /* IF ISA interface THEN*/
       birddata += buttonmode; /* button value is a word,
                                          so inc more */
...
int getmaxrs232tofbbrate()
{
    ...
    */
    if (stopaddr == 0)
       stopaddr = (unsigned char)(flocksize);
