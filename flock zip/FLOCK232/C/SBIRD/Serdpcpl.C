...
1/9/96   vk..global phaseerror_count declaration removed, as it's declared
             in iface.c now.
...
int configserialport(void)
{
    ...
    */
    ...
    OUTPORTB(com_base, (int)(divisor & 0xff));
    OUTPORTB(com_base + 1, (int)((divisor & 0xff00) >> 8));
...
short cmdsize;
{
    ...
    /*
        Send the RS232 to FBB Prefix Character if non-zero
    */
    if (rs232tofbbaddr > 0)
    {  
    	if(enhancedModeFlag && (addressMode == SUPER_EXTENDED))
    		{ 
    		while(send_serial_char(0xA0)==TXNOTEMPTY); 
    		rs232tofbbcmd = (unsigned char)(0xFF & rs232tofbbaddr);
...
int get_serial_char()
{
    ...
    */
    if ((linestatus = INPORTB(com_base + LINESTATUS)) & DATARDY)
    {
        ...
        */ 
        returnChar = (unsigned int)(INPORTB(com_base + RXBUF)) & 0xFF;
        return((int)(returnChar));
    }
    else
    {
        /*
...
//printf("### waitforchar() RXTIMEOUT(%04x)\n\r", rxchar);  
//exit(1);
            return(RXTIMEOUT);
        }
    }
    ...
    */
    if (rxchar < 0)
    	{
//printf("### waitforchar() RXERRORS(%04x)\n\r", rxchar);   
//exit(1);
        return(RXERRORS); 
        }
